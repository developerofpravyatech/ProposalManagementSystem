...
import { Eye, Download, CheckCircle, Smartphone, Monitor, Clock, ShieldAlert } from 'lucide-react';
  ...
  const getActionDetails = (action) => {
    switch (action) {
      ...
      default:
        return {
          icon: Eye,
          color: 'text-brand-400 bg-brand-500/15 border-brand-500/30',
  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-center justify-between border-b border-white/5 pb-3">
        ...
        <span className="text-xs text-slate-400">Silent Real-time Tracker</span>
      </div>
        ...
        <div className="space-y-3">
          {activities.map((act) => {
            ...
            return (
              <div
                key={act.id}
                className="flex items-start justify-between gap-3 p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 transition-all text-xs"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-slate-400 font-mono text-[11px]">{act.proposal_number}</span>
                    </div>
                    <p className="text-slate-300 mt-0.5">{details.label}</p>
